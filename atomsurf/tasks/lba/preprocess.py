...
class ExtractMFEData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            score = self.res[item]
            G = Mult_graph(os.path.join(self.datadir,item,item+'_ligand.mol2'), os.path.join(self.datadir,item,item+'_pocket.pdb'), item, score)
...
if __name__=='main':
    ...
    testpdbbind=PreProcessPDBbindDataset(data_dir=datadir, idxpath=idxpath,out_dir=outputdir,recompute_s=False, recompute_g=False,max_vert_number=100000, face_reduction_rate=0.1, use_pymesh=False)
    ...
    train_list_refine=[]
    train_list_general=[]
    pdblist = GetPDBDict(Path='/work/lpdi/users/ymiao/code/pdbbind/general-set-except-refined/index/INDEX_general_PL_data.2016')
