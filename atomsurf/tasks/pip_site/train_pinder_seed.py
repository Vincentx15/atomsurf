...
from pl_model import PINDERModule_seed,PINDERModule_seed_onfly
from data_loader import PINDERDataModule,PINDERDataModule_atomgraph_onfly,PINDERDataModule_resgraph_onfly
...
def main(cfg=None):
    ...
    # init datamodule
    datamodule = PINDERDataModule_resgraph_onfly(cfg)

    # init model
    model = PINDERModule_seed(cfg) #PINDERModule_seed
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filename="{epoch}-{auroc_site:.2f}",
        dirpath=Path(tb_logger.log_dir) / "checkpoints",
        monitor="auroc_site/val",
        mode="max",
        save_last=True,
    ...
    early_stop_callback = pl.callbacks.EarlyStopping(monitor='auroc_site/val',
