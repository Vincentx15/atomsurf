...
from data_loader import RFPPIDataModule
...
def main(cfg=None):
    ...
    # init datamodule
    datamodule = RFPPIDataModule(cfg)
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filename="epoch={epoch:02d}-auprc_val={auprc_ppi/val:.4f}",  # 使用 4 位小数
        dirpath=Path(tb_logger.log_dir) / "checkpoints",
        monitor="auprc_ppi/val",
        mode="max",
        save_last=True,
...
if __name__ == "__main__":
    ...
    main()
# cd /work/lpdi/users/ymiao/code/newcode_0923/atomsurf/atomsurf/tasks/pip_site/ && python train_rfppi_dataset.py data_dir=/work/lpdi/users/ymiao/code/rfppi_dataset/cfg_graph.use_esm=True optimizer.lr=5e-4 run_name=test_rfppi_dataset device=0 out_dir=/work/lpdi/users/ymiao/code/sbatch/pinderlog  log_dir=/work/lpdi/users/ymiao/code/sbatch/pinderlog/ loader.batch_size=32 loader.num_workers=30 epochs=30 encoder=pronet_hmrencoder use_wandb=False cfg_surface.data_dir=/work/lpdi/users/ymiao/code/rfppi_dataset/ cfg_graph.data_dir=/work/lpdi/users/ymiao/code/rfppi_dataset/ cfg_graph.esm_dir=/work/lpdi/users/ymiao/code/rfppi_dataset/esm  loader.prefetch_factor=5 seed=42